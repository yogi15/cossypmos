...
public class JInternalPositionFrame extends JInternalFrame {
	...
	public void setReportPanel(ReportPanel reportPanel) {
		...
		loadJobs(reportPanel.getUserJobsDetails());
	...
	public JInternalPositionFrame(PositionFilterValues filterValues,Vector<StartUPData> searchCriteria,Vector<StartUPData> searchColumn,String reportType) {
	...
	private JButton getJButton0() {
		if (jButton0 == null) {
			...
			public void mouseClicked(MouseEvent event) {
				String sql = "";
				filters = searchPanel.getData();
				String where =  getFilterValues().createWhere(filters,"Position");
				
				try {
					//commonUTIL.showAlertMessage(sql);
			
				data = (Vector<Position>)	 reportPanel.getRemoteMO().getPositionOnWhere(where, null);
				if(data != null && data.size() > 0) {
					
					reportPanel.loadreport(data);
	 ...
	 */
	private PositionFilterValues getFilterValues() {
		return filterValues;
	}
    ...
    public Vector<FilterBean> getFilters() {
    	return filters;
    }
