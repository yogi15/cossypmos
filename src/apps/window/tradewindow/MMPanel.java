...
public class MMPanel extends CommonPanel {
	...
	DefaultCellEditor compF = null;
	DefaultCellEditor compoundingType = null;
	AmortizationPanel amortizatwindow = null;
	JComboBox compoundType = null;
	JComboBox compFreq = null;
	Vector v1 = null;
	Vector ve1 = null;
	Vector ve2 = null;
	Vector ve3 = null;
	...
	public MMPanel() {
		...
		try {
			...
			ve8 = (Vector) referenceData.getStartUPData("Compounding");
		...
		final JComboBox comboBox11 = new JComboBox(
				convertVectortoSringArray(ve5));
		  compoundType = new JComboBox(
				convertVectortoSringArray(ve8));
		compFreq = new JComboBox(convertVectortoSringArray(ve5));

		JComboBox comboBox21 = new JComboBox(
				convertVectortoSringArray(dateRoll));
		comboBox21.setSelectedItem(new String("FOLLOWING"));
		JComboBox Period = new JComboBox(convertVectortoSringArray(ve3));

		Amortizing = new JComboBox(convertVectortoSringArray(ve8));
		// Amortizing.setSelectedItem(new String("None"));
		Amortizing.addItemListener(new ItemListener() {

			@Override
			public void itemStateChanged(ItemEvent e) {
				amortizationItemEventChange();
			
				if( !Amortizing.getSelectedItem().toString().equalsIgnoreCase("NA")) {
					compoundType = new JComboBox(convertVectortoSringArray(ve5));
					compoundType.setSelectedItem("PA");
					compoundingType = new DefaultCellEditor(compoundType);
					t1model.setValueAt("CompoundingFrequency", 6, 0);
					t1model.setValueAt("PA", 6, 1);
					compoundType.setEnabled(true);
					t1model.setValueAt(null, 7, 0);
					t1model.setValueAt(null, 7, 1);
					compFreq.setEnabled(false);
				
				} else {
					t1model.setValueAt(CommonConstants.BLANKSTRING, 6, 0);
					t1model.setValueAt(CommonConstants.BLANKSTRING, 6, 1);
				}
			}
		});
		comboBox11.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				if (comboBox11 == null)
					return;
				amortizatwindow.jTable1.setValueAt(comboBox11.getSelectedItem()
						.toString(), 4, 1);
				amortizatwindow.jTable1.setValueAt(jTable2.getValueAt(2, 1)
						.toString(), 3, 1);
		...
		compF = new DefaultCellEditor(compFreq);
		compoundingType = new  DefaultCellEditor(compoundType);
		Amortizing.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
		...
		tenorcomboBox11.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				amortizatwindow.jTable1.setValueAt(
						commonUTIL.getDateFormat(dateissueDate.getDate()), 2, 1);
			}
	...
	private void amortizationItemEventChange() {
		amortizatwindow.jTable1.setValueAt(Amortizing.getSelectedItem()
			...
			jTable2 = new javax.swing.JTable(tradePrice) {
				...
				public TableCellEditor getCellEditor(int row, int column) {
					...
					if (modelColumn == 1 && (row == 5)) {
						TableCellEditor t1 = ((TableCellEditor) compF);
						return (TableCellEditor) t1;
					}
			...
			jTable1 = jTable1 = new javax.swing.JTable(t1model) {
				public TableCellEditor getCellEditor(int row, int column) {
					...
					if (modelColumn == 1 && (row == 6)) {
						TableCellEditor t1 = ((TableCellEditor) compoundingType);
						return (TableCellEditor) t1;
					} 
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (product != null) {
					...
					if (!validateProductData()) {
						...
						trade.setTradeDate(tradevalue.tradeDate.getText());
						///trade.setDelivertyDate(tradevalue.tsettlement.getText());
						trade.setDelivertyDate(jTable1.getValueAt(1, 0).toString());
						trade.setEffectiveDate(jTable1.getValueAt(1, 0).toString());
		...
		buysell.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (buysell.getText().equalsIgnoreCase("LOAN")) {
					...
					buildAmortizationData(TradeConstants.TRADETYPE_SELL);
					...
					Amortizing.addItemListener(new ItemListener() {
						...
						public void itemStateChanged(ItemEvent e) {
							amortizationItemEventChange();
							if(Amortizing.getSelectedItem().toString().equalsIgnoreCase("NONE")) {
								compoundType = new JComboBox(convertVectortoSringArray(ve8));
								compoundType.setSelectedItem("NA");
								compoundingType = new DefaultCellEditor(compoundType);
								compFreq = new JComboBox(convertVectortoSringArray(ve5));
								compFreq.setSelectedItem("PA");
								compF = new DefaultCellEditor(compFreq);
								t1model.setValueAt("Compounding", 6, 0);
								t1model.setValueAt("NA", 6, 1);
								compoundType.setEnabled(true);
								t1model.setValueAt("CompoundingFrequency", 7, 0);
								t1model.setValueAt("PA", 7, 1);
								compFreq.setEnabled(true);
							} else {
							t1model.setValueAt(null, 6, 0);
							t1model.setValueAt(null, 6, 1);
							compoundType.setEnabled(false);
							t1model.setValueAt(null, 7, 0);
							t1model.setValueAt(null, 7, 1);
							compFreq.setEnabled(false);
							}
				...
				} else if (buysell.getText().equalsIgnoreCase("DEPOSIT")) {
					
					buildAmortizationData(TradeConstants.TRADETYPE_BUY);
	...
	private void setTableValuesBlank() {
		...
		t1model.insertRow(t1model.getRowCount(), new Object[] { "Compounding",
				"" });
		...
		jTable1.setValueAt("FOLLOWING", 3, 1);
		jTable1.setValueAt("NA", 5, 1);
		jTable2.setValueAt("INR", 1, 1);
		jTable2.setValueAt("ACT/360", 3, 1);
		jTable2.setValueAt("Fixed", 0, 1);
	...
	public boolean validateProductData() {
		...
		String tenor = jTable2.getValueAt(4, 1).toString();
		...
		if (!freq.equalsIgnoreCase("ZC")) {
			...
			String type = jTable1.getValueAt(5, 1).toString();
			
			if (!type.equals(MMConstants.NACOMPOUNDINGTYPE) && type.equals(MMConstants.NONEAMORTYPE)) {
				
				if (jTable1.getValueAt(6, 1).toString() != null && !jTable1.getValueAt(6, 1).toString().equals(MMConstants.NACOMPOUNDINGTYPE) ) {
					// for loan compounding frequency value will be fetch from 7th row 
					// where as for deposit; compounding frequency value will be fetch from 6th row
					
					String compoundingFrequency = jTable1.getValueAt(6, 1).toString().equals(MMConstants.SIMPLECOMPOUNDINGAMORTYPE)?jTable1.getValueAt(7, 1).toString():jTable1.getValueAt(6, 1).toString();

					if (commonUTIL.isEmpty(compoundingFrequency)) {
						commonUTIL.showAlertMessage("Select frequency ");
						return flag;
					}
					
					if (DateConstants.getCompareFrequencyCode(compoundingFrequency) > DateConstants
							.getCompareFrequencyCode(freq)) {
						commonUTIL
		...
		if (actionType.equalsIgnoreCase("NEW")) {
			...
			try {
				...
				if (productId > 0) {
					...
					trade.setCurrency(coupon.getCCY());
					...
					amortizatwindow.jTable1.setValueAt(
							(String) jTable1.getValueAt(5, 1), 0, 1);
					amortizatwindow.jTable1.setValueAt(
							(String) jTable2.getValueAt(2, 1), 3, 1);
	...
	private void validateAmortization() {
		// TODO Auto-generated method stub
		StringBuffer amort = null;
		String amortType = (String) amortizatwindow.jTable1.getValueAt(0, 1);
		String startDate = (String) amortizatwindow.jTable1.getValueAt(1, 1);
		String endDate = (String) amortizatwindow.jTable1.getValueAt(2, 1);
		String rate = (String) amortizatwindow.jTable1.getValueAt(3, 1);
		String freq = (String) amortizatwindow.jTable1.getValueAt(4, 1);
		
		String openTerm = "false";
		String loanTerm = "false";
		if (amortizatwindow.jCheckBox1.isSelected())
			openTerm = "true";
		if (amortizatwindow.jCheckBox2.isSelected())
			loanTerm = "true";
		...
		if (commonUTIL.isEmpty(startDate)
				|| (!commonUTIL.isStringDate(startDate))) {
			commonUTIL.showAlertMessage("Select Start Date  ");
			return;
		}
		if (commonUTIL.isEmpty(endDate) || (!commonUTIL.isStringDate(endDate))) {
		...
		if (commonUTIL.isEmpty(freq)) {
			...
			if (jTable1.getValueAt(5, 1) != null && !jTable1.getValueAt(5, 1).equals("")) {
				
				if (jTable1.getValueAt(5, 1).equals(MMConstants.NACOMPOUNDINGTYPE)) {
					
					amort
					.append(";amortizingtype=")
					.append(jTable1.getValueAt(5, 1))
					.append(";amortizationValue=")
					.append("0")
					.append(amortizatwindow.getScheduledValues());
											
				} else if (jTable1.getValueAt(5, 1).equals(MMConstants.SIMPLECOMPOUNDINGAMORTYPE)) {
					
					amort
					.append(";amortizingtype=")
					.append(jTable1.getValueAt(5, 1))
					.append(";amortizationValue=")
					.append(jTable1.getValueAt(6, 1))
					.append(amortizatwindow.getScheduledValues());
					
					
				} else if (jTable1.getValueAt(5, 1).equals(MMConstants.NONEAMORTYPE)) {
					
					if (jTable1.getValueAt(6, 1).equals(MMConstants.NACOMPOUNDINGTYPE)) {
				...
				/*	if (jTable1.getValueAt(5, 1) != null && !jTable1.getValueAt(5, 1).equals("")) {
						...
						if (jTable1.getValueAt(5, 1).equals(MMConstants.NONEAMORTYPE)) {
							
							amort
							.append(";amortizingtype=")
							.append(jTable1.getValueAt(6, 1))
							.append(";amortizationValue=")
							.append(jTable1.getValueAt(6, 1).equals(MMConstants.NACOMPOUNDINGTYPE)?"0":jTable1.getValueAt(7, 1))
							.append(amortizatwindow.getScheduledValues());	
							...
							.append(jTable1.getValueAt(5, 1))
							.append(";amortizationValue=")
							.append((jTable1.getValueAt(5, 1).equals(MMConstants.NACOMPOUNDINGTYPE)
									)?"0":jTable1.getValueAt(6, 1))
							.append(amortizatwindow.getScheduledValues());
						...
						.append(";amortizationValue=")
						.append(jTable1.getValueAt(5, 1).equals(MMConstants.NACOMPOUNDINGTYPE) ||
								jTable1.getValueAt(6, 1)==null ||
								jTable1.getValueAt(6, 1).equals(CommonConstants.BLANKSTRING) ||
								!jTable1.getValueAt(6, 1).equals(MMConstants.NONEAMORTYPE)?"0":jTable1.getValueAt(6, 1) )			
						.append(jTable1.getValueAt(5, 1).equals(MMConstants.NACOMPOUNDINGTYPE)||
								!jTable1.getValueAt(5, 1).equals(MMConstants.NONEAMORTYPE)?"0":CommonConstants.BLANKSTRING )
						.append(amortizatwindow.getScheduledValues());
						
					}
					*/
					
			amortizatwindow.setAmoritization(amort.toString());
	...
	public void openTrade(Trade trade) {
		...
		setTableValues(getProdcutToOpenTrade(trade.getProductId(), productID), trade);
		bondProduct.setText(product.getProductname());
	...
	public void setTableValues(Product product, Trade trade) {
		...
		setAmortizationData(trade.getAmoritizationData());
		setTrade(trade);
		buildAmortizationData(trade.getType());
		// trade price table
		jTable2.setValueAt(coupon.getCouponType(), 0, 1);
		jTable2.setValueAt(coupon.getCCY(), 1, 1);
		jTable2.setValueAt(
				commonUTIL.converDoubleToString(coupon.getFixedRate()), 2, 1);
		jTable2.setValueAt(coupon.getDayCount(), 3, 1);
		jTable2.setValueAt(product.getTenor(), 4, 1);
		
		// below statements have been commented as cmp frq and Sales margin is not shown in trade price table
		/*jTable2.setValueAt(new String(""), 4, 1);
		jTable2.setValueAt(new String(""), 5, 1);*/
	...
	private void buildAmortizationData(String tradeType) {
		
		String amortizationData =  getAmortizationData();
		
		Trade trade = getTrade();
				
		if (!amortizationData.equals(CommonConstants.BLANKSTRING) && tradeType.equals(trade.getType())) {
			
			String type =  Miscellaneous.getAmortizationVal(amortizationData, "Type" );
			String compundingTypeVal = Miscellaneous.getAmortizationVal(amortizationData, "amortizingtype" );
			
			if ( type.equals(MMConstants.NONEAMORTYPE) && compundingTypeVal.equals(MMConstants.NACOMPOUNDINGTYPE)) {
				//NONE NA 0
				
				jTable1.setValueAt("Amortizing", 5, 0);
				jTable1.setValueAt(type, 5, 1);
				jTable1.setValueAt(MMConstants.COMPOUNDING_STR, 6, 0);
				jTable1.setValueAt(compundingTypeVal, 6, 1);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 7, 0);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 7, 1);
				
			} else if ( type.equals(MMConstants.MORTGAGEAMORTYPE) && compundingTypeVal.equals(MMConstants.MORTGAGEAMORTYPE)) {
				//MORT MORT 0
				
				jTable1.setValueAt("Amortizing", 5, 0);
				jTable1.setValueAt(type, 5, 1);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 6, 0);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 6, 1);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 7, 0);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 7, 1);
				
			} else if ( type.equals(MMConstants.NONEAMORTYPE) && compundingTypeVal.equals(MMConstants.SIMPLECOMPOUNDINGAMORTYPE)) {
				// NONE SC QTR
				
				String compundingFrequency = Miscellaneous.getAmortizationVal(amortizationData, "amortizationValue" ); 
				
				jTable1.setValueAt("Amortizing", 5, 0);
				jTable1.setValueAt(type, 5, 1);
				jTable1.setValueAt(MMConstants.COMPOUNDING_STR, 6, 0);
				jTable1.setValueAt(MMConstants.SIMPLECOMPOUNDINGAMORTYPE, 6, 1);
				jTable1.setValueAt("Compounding Freqency", 7, 0);
				jTable1.setValueAt(compundingFrequency, 7, 1);
				
			} else if ( type.equals(MMConstants.NACOMPOUNDINGTYPE) && compundingTypeVal.equals(MMConstants.NACOMPOUNDINGTYPE)) {
				//NA NA 0
			
				jTable1.setValueAt(MMConstants.COMPOUNDING_STR, 5, 0);
				jTable1.setValueAt(type, 5, 1);
				jTable1.setValueAt("Compounding Frequency", 6, 0);
				jTable1.setValueAt(CommonConstants.BLANKSTRING, 6, 1);
				...
				jTable1.setValueAt(type, 5, 1);
				jTable1.setValueAt("Compounding Frequency", 6, 0);
				jTable1.setValueAt(compundingFrequency, 6, 1);
		
					
				}		
				
		} else if (tradeType.equals(TradeConstants.TRADETYPE_BUY)) {
			
			t1model.setValueAt(CommonConstants.BLANKSTRING, 6, 0);
			t1model.setValueAt(CommonConstants.BLANKSTRING, 6, 1);
			t1model.setValueAt(CommonConstants.BLANKSTRING, 7, 0);
			t1model.setValueAt(CommonConstants.BLANKSTRING, 7, 1);
			
			Amortizing.removeAll();
			Amortizing = null;
			amortizingdce13 = null;
			Amortizing = new JComboBox(convertVectortoSringArray(ve8));
			
			jTable1.setValueAt("Compounding", 5, 0);
			jTable1.setValueAt("NA", 5, 1);
			...
			Amortizing.removeAll();
			Amortizing = null;
			Amortizing = new JComboBox(convertVectortoSringArray(ve4));
			Amortizing.addItem("NONE");
			amortizingdce13 = null;
			Amortizing.setSelectedItem("NONE");
			jTable1.setValueAt("Amortizing", 5, 0);
			jTable1.setValueAt("None", 5, 1);
	...
	private String getAmortizationData() {
		
		return amortizationData;
		
	}
	
	private void setAmortizationData(String amortizationData) {
