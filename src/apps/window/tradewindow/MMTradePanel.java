...
public class MMTradePanel extends TradePanel {
	...
	public javax.swing.JTextField getTsettlement() {
		return tsettlement;
	}

	public NumericTextField tquantity;

	public javax.swing.JTextField getTquantity() {
		return tquantity;
	}
	...
	private void initComponents() {
		...
		add(getJPanel0(), new Constraints(new Bilateral(8, 10, 1146),
				new Bilateral(6, 20, 10)));
		setSize(1164, 230);
	}
	...
	private JPanel getJPanel1() {
		if (jPanel1 == null) {
			...
			jPanel1.add(getJScrollPane0(), new Constraints(new Leading(12, 391,
					10, 10), new Leading(7, 174, 10, 10)));
		}
		return jPanel1;
	}
			...
			public void keyTyped(KeyEvent e) {
				if (trade != null) {
					if (trade.getProductType().equalsIgnoreCase("MM")) {
						MMPricing pricing = (MMPricing) productWindowpanel
								.getPricer();
						// pricing.calculateYield(new
						// Double(tprice.getText()).doubleValue());
						tradeyield.setText(commonUTIL.doubleFormat(pricing
								.getYield()));
					}
	...
	private JTextField getTradedate() {
		if (tradeDate == null) {
			...
			tradeDate.setText((commonUTIL.getDateFormat(commonUTIL
					.getCurrentDate())));
			tradeDate.setEditable(false);
			tradeDate.setVisible(false);
		}
		return tradeDate;
	}
	...
	private JComboBox getTcurrency() {
		if (tcurrency == null) {
			...
			tcurrency.setModel(new DefaultComboBoxModel(new Object[] { "INR",
	...
	private JComboBox getJComboBox4() {
		if (tcurrency == null) {
			...
			tcurrency.setSelectedItem(new String("INR"));
			tcurrency.setModel(new DefaultComboBoxModel(new Object[] { "item0",
					"item1", "item2", "item3" }));
		}
		return tcurrency;
	}
			...
			public void keyTyped(KeyEvent e) {
				if (e.getKeyChar() == KeyEvent.VK_ENTER) {
					try {
						...
						if (trade != null) {
							if (!trade.getProductType().equalsIgnoreCase(
									productType)) {
								commonUTIL
										.showAlertMessage("This Trade Will not open in current Window");
							}
							setTrade(trade);
							openTrade(trade);
							...
							commonUTIL
									.showAlertMessage("Trade does not Exists with id ");
						}
						...
						System.out
								.println("JFrameTradeWindowApplication  " + e);
					}
				}
			}
		});
	...
	private JPanel getJPanel0() {
		if (jPanel0 == null) {
			...
			jPanel0.add(getJLabel0(), new Constraints(new Leading(12, 95, 10,
					10), new Leading(22, 21, 12, 12)));
			jPanel0.add(getJLabel1(), new Constraints(new Leading(12, 95, 40,
					469), new Leading(70, 21, 12, 12)));
			jPanel0.add(getJLabel2(), new Constraints(new Leading(12, 95, 40,
					469), new Leading(114, 21, 10, 10)));
			jPanel0.add(getTradedate(), new Constraints(new Leading(406, 127,
					46, 314), new Leading(159, 27, 12, 12)));
			jPanel0.add(getJLabel5(), new Constraints(new Leading(12, 82, 10,
					10), new Leading(162, 21, 12, 12)));
			jPanel0.add(getTprice(), new Constraints(new Leading(406, 127, 46,
					314), new Leading(64, 27, 12, 12)));
			jPanel0.add(getJLabel6(), new Constraints(new Leading(360, 76, 10,
					10), new Leading(19, 21, 12, 12)));
			jPanel0.add(getJLabel7(), new Constraints(new Leading(360, 82, 10,
					10), new Leading(66, 21, 12, 12)));
			jPanel0.add(getJLabel3(), new Constraints(new Leading(360, 48, 12,
					12), new Leading(159, 21, 12, 12)));
			jPanel0.add(getJcp(), new Constraints(
					new Leading(131, 183, 10, 10), new Leading(16, 27, 12, 12)));
			jPanel0.add(getTtradername(), new Constraints(new Leading(131, 182,
					12, 12), new Leading(108, 27, 12, 12)));
			jPanel0.add(getTradeamount(), new Constraints(new Leading(131, 182,
					12, 12), new Leading(156, 27, 12, 12)));
			jPanel0.add(getBook(), new Constraints(
					new Leading(131, 182, 12, 12), new Leading(64, 27, 12, 12)));
			jPanel0.add(getTuse(), new Constraints(
					new Leading(459, 162, 10, 10), new Leading(16, 27, 12, 12)));
			jPanel0.add(getTradeid(), new Constraints(new Leading(456, 168, 12,
					12), new Leading(64, 27, 12, 12)));
			jPanel0.add(getTradestatus(), new Constraints(new Leading(456, 168,
					12, 12), new Leading(159, 27, 12, 12)));
			jPanel0.add(getCaction(), new Constraints(new Leading(456, 168, 12,
					12), new Leading(114, 27, 12, 12)));
			jPanel0.add(getJLabel4(), new Constraints(new Leading(357, 49, 10,
					10), new Leading(114, 21, 12, 12)));
			jPanel0.add(getJPanel1(), new Constraints(new Leading(704, 419, 10,
					10), new Leading(9, 188, 10, 10)));
	...
	private JTable getJTable0() {
		if (JTable1 == null) {
			JTable1 = new JTable();
			String attributeColumnName[] = { "Attribute Name ",
					"Attribute  Value " };
		...
		if (attributes != null && attributes.length() > 0) {
			...
			for (int i = 0; i < atttoken.length; i++) {
				...
				if (att.contains("=")) {
					...
					attributeModel
							.insertRow(i, new Object[] { attnameName.trim(),
									attvalue.trim() });
				}
			}
			JTable1.removeAll();
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (tradeAttributes.getName().equalsIgnoreCase("Trade Date")) {
					model.insertRow(i,
							new Object[] { tradeAttributes.getName(),
									commonUTIL.getCurrentDateTime() });
				} else {
					model.insertRow(i, new Object[] {
							tradeAttributes.getName(), "0" });
				}
	...
	public void processDataCombo1(javax.swing.DefaultComboBoxModel combodata,
			Hashtable ids, String name, String leRole) {
		Vector vector;
		try {
			if (name.equalsIgnoreCase("Book"))
				vector = (Vector) referenceData.selectALLBooks();
			else
				vector = (Vector) referenceData
						.selectLEonWhereClause(" role like '%" + leRole + "%'");
			Iterator it = vector.iterator();
			int i = 0;
			while (it.hasNext()) {
	...
	public void saveASNew(Trade trade) {
		...
		// / imp note : wf understand CASH productType as MM so passing MM as
		// hardcoded
		processActionData(actionstatus, "MM", trade.getTradedesc1(),
				tradestatus.getText(), rtradeservices);
		this.trade = trade;
	...
	public void buildTrade(Trade trade, String actionType) {
		...
		if (actionType.equalsIgnoreCase("NEW")) {
			...
			// currency and yeild field are not shown in MM trade panel
			// tcurrency.setSelectedItem(new String("INR"));
			...
			trade.setCpID(((LegalEntity) cpData.get(jcp.getSelectedIndex()))
					.getId());
			trade.setTraderID((((LegalEntity) trader.get(ttradername
					.getSelectedIndex())).getId()));
			trade.setBookId(((Book) bookData.get(book.getSelectedIndex()))
					.getBookno());
			...
			// trade currency is set in MM panel after validatProduct()
			// trade.setCurrency((String)tcurrency.getSelectedItem());
			
			// yeild is not shown in the Trade panel so it has to be commented below
	...
	public void openTrade(Trade trade) {
		...
		tradeid.setText(new Integer(trade.getId()).toString());
		// trade.setCpID(((LegalEntity)
		// cpData.get(jcp.getSelectedIndex())).getId());
		// trade.setBookId(((Book)
		// bookData.get(book.getSelectedIndex())).getBookno());
		tradestatus.setText(trade.getStatus());
		...
		// currency field and yeils field are not shown in MM
	...
	private String getAttributeValue() {
		...
		for (int i = 0; i < JTable1.getRowCount(); i++) {
			...
			if (attributeValue != null && attributeValue.length() > 0)
				attributes = attributes.trim() + attributename + "="
						+ attributeValue + ";";
	...
	public void getTradeTask(BackOfficePanel panel) {
		try {
			...
			panel.fillJTabel((Vector) remoteTask.selectTaskWhere("tradeId = "
					+ trade.getId()));
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void getTradeTransfers(BackOfficePanel panel) {
		try {
			...
			panel.fillJTabel((Vector) remoteBO.queryWhere("Transfer",
					"tradeId = " + trade.getId()));
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
