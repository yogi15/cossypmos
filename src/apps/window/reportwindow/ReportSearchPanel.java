...
import apps.window.operationwindow.jobpanl.DataPanel;

import apps.window.operationwindow.jobpanl.FilterValues;
import apps.window.operationwindow.jobpanl.JTableX;
import apps.window.operationwindow.jobpanl.JobDatePanel;
import apps.window.operationwindow.jobpanl.ReportPanel;
import apps.window.operationwindow.jobpanl.RowEditorModel;
...
public class ReportSearchPanel extends JPanel {
	...
	public ReportSearchPanel(String name,Vector searchCriteria,Vector searchColumn,FilterValues filterValues,UserJob job, Vector<UserJobsDetails> jobd,RemoteTask remoteTask,RemoteTrade remoteTrade,Users user,RemoteReferenceData remoteRef) {
			...
			final    Renderer renderer = new Renderer(manager);
	         ...
	         final	JComboBox searchCriteriaComb  = new JComboBox(modelC);
	         	 ...
	         	 DefaultCellEditor searchCriteriaCedd = new DefaultCellEditor(searchCriteriaComb);
	         	    	...
	         	    	DefaultCellEditor valuesCed = null;
	         	    	if(dataValues == null || dataValues.isEmpty()) {
                	  		final JTextField numberValues   = new JTextField();
                	  		valuesCed = new DefaultCellEditor(numberValues);
                	  			
                	  		numberValues.addMouseListener(new MouseAdapter()  {
                	  			public void mouseClicked(MouseEvent event) {
                	  				selectRow = jTable0.getSelectedRow();
                	  				selectCol = jTable0.getSelectedColumn();
                	  			}
							});
                	  	...
                	  	numberValues.addFocusListener(new FocusAdapter() {
                	  		public void focusLost(FocusEvent event) {
                                 ...
                                 try {	
                                   ...
                                   int i = selectRow;
                                   commonUTIL.showAlertMessage(String.valueOf(i));
                                   FilterBean filler = (FilterBean) filters.get(selectRow);
	           	  	...
	           	  	valuesCed = new DefaultCellEditor(comvalues);
	...
	private JButton getJButton0() {
		if (jButton0 == null) {
			...
			//jButton0.setText("B1");
			ImageIcon icon = new ImageIcon("/resources/icon/clearall.png");
			jButton0.setIcon(icon);
	...
	private JLabel getJLabel1() {
		if (jLabel1 == null) {
			...
			//jLabel1.setText(" L2");
			ImageIcon icon = new ImageIcon(this.getClass().getResource("/resources/icon/hide.png"));
	...
	private JLabel getJLabel0() {
		if (jLabel0 == null) {
			...
			public void mouseClicked(MouseEvent event) {
				...
				String where =  filtersValues.createWhere(filters,getReportType());
				String sqlW = "";
			...
			//	commonUTIL.showAlertMessage(sqlW);
			//	String startDate = (String) datePanel.jTable1.getValueAt(0, 1);
			//	String endDate = (String) datePanel.jTable1.getValueAt(0, 3);
				
				sql = sqlW;//generateSQL(sqlW,startDate,endDate,jobdataPanel);// tobe done on timestamp
			//	sql = " id > 1";
				try {
					//commonUTIL.showAlertMessage(sql);
				data = (Vector)	 remoteTrade.getTradesforReport(sql);
				if(data != null && data.size() > 0) {
					pReport.setHeader((Vector) data.get(0)); 
					pReport.setdatatype((Vector) data.get(1));
					pReport.setData((Vector) data.get(2));
	...
	private String  generateSQL(String where,String StartDate,String endDate,DataPanel jobdetPanel) {
		...
		if(EndDate.isEmpty() || (startDate.equalsIgnoreCase(EndDate)))  {
			sql = where + " and to_date(trade.tradedate,'dd/mm/yyyy') >= to_date('" +startDate +"','dd/mm/yyyy')";
		...
		if(!EndDate.isEmpty() ) {
			...
			sql =   where + " and to_date(trade.tradedate,'dd/MM/yyyy')   between to_date('"+startDate +"','dd/MM/yyyy') and to_date('" + EndDate +"','dd/MM/yyyy')";
		}
		return sql;
	...
	public void addReportPanel(JPanel panel) {
	...
	private JPanel getJPanel3() {
		...
		if (jPanel3 == null) {
			jPanel3 = new JPanel();
		//	datePanel = new JobDatePanel("Trade");
		...
		//	jPanel3.add(getJSplitPane1());//, new Constraints(new Bilateral(6, 6, 473), new Bilateral(6, 5, 10, 412)));
			...
			//jPanel3.add(reportPanel.getJPanel5(datePanel),BorderLayout.CENTER);
		}
		return jPanel3;
	}
	private JPanel getJSplitPane1() {
		...
		if(reportPanel == null) {
	 	...
	 	panel.setLayout(new BorderLayout());
			jPanel3.setBorder(new LineBorder(Color.black, 1, false));
			datePanel = new DatePanel("Trade");
			panel.add(reportPanel.getJPanel5(datePanel),BorderLayout.CENTER);
	...
	//		jSplitPane1.setOrientation(JSplitPane.VERTICAL_SPLIT);
		...
		//	jSplitPane1.setBottomComponent(reportPanel.getJPanel5(datePanel));
...
if(setDataCriteria) {
	/*initialStart();
	jobdataPanel = new DataPanel(data,filtersValues);	
...
public void processTask(TaskEventProcessor task) {
	// TODO Auto-generated method stub
	System.out.println(" Name " + name + " "+ task.getTaskID());
