...
import apps.window.positionwindow.LiquidationPanel;
	...
	//	positionEvent.setTradeID(trade.getId());
		try {
			if(remoteTrade == null) {
					de =ServerConnectionUtil.connect("localhost", 1099,commonUTIL.getServerIP() );
				remoteTrade = (RemoteTrade)  de.getRMIService("Trade");
				remoteTrade.publishnewTrade("TRANS_NEWTRANSFER","TRADE", positionEvent);
			} else {
	...
	public void publishnewPosition(String messageIndicator, String messageType,
		...
		try{   
			if(getNewMessage() == null) {
					...
					newMessage.produceNewMessage(messageIndicator,"POSITION",messageType,(Serializable) object,"true"); 
			} else {
			
			newMessage.produceNewMessage(messageIndicator,"POSITION",messageType,(Serializable) object,null); 
			//newMessage.run();
			}
			}catch(Exception e){
